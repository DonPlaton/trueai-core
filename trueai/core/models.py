...
#: What a finding of each evidence class does *not* establish, one sentence each.
#: Kept beside the enum rather than inside one renderer, because every surface
#: that shows a finding owes the reader the same sentence and two copies of these
#: in particular would be the worst thing in the project to let drift.
CONFIDENCE_LIMITS: Final[dict[ConfidenceType, str]] = {
...
#: The same, for the provenance attribute a finding may also carry. `NONE` is
#: absent on purpose: there is no provenance relationship to qualify.
PROVENANCE_LIMITS: Final[dict[ProvenanceClass, str]] = {
...
def evidence_limits(
    confidence_type: ConfidenceType, provenance_class: ProvenanceClass
) -> tuple[str, ...]:
    """Return every caveat that applies to a finding with these two classes."""

    provenance = PROVENANCE_LIMITS.get(provenance_class)
    if provenance is None:
        return (CONFIDENCE_LIMITS[confidence_type],)
    return (CONFIDENCE_LIMITS[confidence_type], provenance)
